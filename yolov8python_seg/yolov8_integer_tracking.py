...
class YOLOV8:
    ...
    def detect(self, image, object=None):
        ...
        for i in results:
            ...
            if ((detected.shape[0]*detected.shape[1])>size_threshold)and(i[5]==0):
                random_number = random.randint(1, 1000000) 
                filename = f"./out/{random_number}.png"
                brightness_factor = 1.5  # You can adjust this factor as needed
                detected_pil = Image.fromarray(detected)
                enhancer = ImageEnhance.Brightness(detected_pil)
                detected_final = enhancer.enhance(brightness_factor)
                detected_final_np = np.array(detected_final)


                
                cv.imwrite(filename, detected_final_np)
...
#             # plotter.plot_bboxes(file, results)




######second way of background removed
...
#             # Remove background
#             segmented_img = remove_background(file)

#             # Process the segmented image with YOLOV8
#             # yolo_output = yolo.process(segmented_img)

#             # Process bounding boxes and plot them on the segmented image
#             # bboxes = process_yolo_output(yolo_output)
#             # output_img = plotter.draw_bboxes(segmented_img, bboxes)

#             # Save the output image
#             output_file = os.path.join(output_dir, os.path.basename(file))
#             cv2.imwrite(output_file, segmented_img)
