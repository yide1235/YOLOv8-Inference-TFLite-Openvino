...
std::vector<vector<float>>* results2, float move_threshold, float ratio_threshold){
  ...
  //result is become 4bbox, 1confidence, 1classid, 1lastseen, 1id, 1iffindforco, 1updatedco/svd score, delta_ratio,     delta_center_x,   delta_center_y
      //update delta ratio and delta center to it
      ...
      if((results1[i][7]==(*results2)[j][7])&&(results1[i][8]!=-1)&&((*results2)[j][8]!=-1)){
        ...
        float delta_size=corresp_ratio-i_ratio;

        //delta size is the ratio


        float i_x_mid=(i_box[2]-i_box[0])/2+i_box[0];
        float i_y_mid=(i_box[3]-i_box[1])/2+i_box[1];

        float corresp_x_mid=(corresp_box[2]-corresp_box[0])/2+corresp_box[0];
        float corresp_y_mid=(corresp_box[3]-corresp_box[1])/2+corresp_box[1];


        float delta_x=corresp_x_mid-i_x_mid;
        float delta_y=corresp_y_mid-i_y_mid;

        (*results2)[j][10]=(results1[i][10]+delta_size);


        (*results2)[j][11]=(results1[i][11]+delta_x);


        (*results2)[j][12]=(results1[i][12]+delta_y);

      }


      if( std::abs((*results2)[j][10]>ratio_threshold || std::abs((*results2)[j][11])>move_threshold || std::abs((*results2)[j][12])>move_threshold)   ){
        results.push_back(static_cast<int>((*results2)[j][7]));
      }
    }
...
//so now the result is become 4bbox, 1confidence, 1classid,   1lastseen frame, 1trackingid, 1iffindforco, 1co/svd score, 1delta_ratio, 1delta_centerx 1delta_centery

void generateIds(std::vector<std::vector<float>>* results, std::vector<int>*id_list, std::vector<float>* myList) {
    for (int i = 0; i < (*results).size(); ++i) {
      (*results)[i].push_back(0.0);  //lastseen
      (*results)[i].push_back(static_cast<float>(i));  //trackingid
      ...
      (*results)[i].push_back(-1.0); //found or not
      (*results)[i].push_back(0.0); //co/svd score

      // //initial the delta to its size and center coord, so the first change will correct
      // std::vector<float> i_box;
      // for(int m=0;m<4;m++){
      //     int each2=(*results)[i][m];
      //     i_box.push_back(each2);
      // }

      // (*results)[i].push_back((i_box[2]-i_box[0])*(i_box[3]-i_box[1])); //delta_ratio, intial with size
      // (*results)[i].push_back((i_box[2]-i_box[0])/2+i_box[0]); //deltacenter x

      // (*results)[i].push_back((i_box[3]-i_box[1])/2+i_box[1]);//delta center y
    ...
    ) 
{
  ...
  //if results1 have -1 that is not found add to reuslts2,
  //but also need to add when its 1, but cannot found corresponding ids.
  for(int i=0; i< (*results1).size(); i++){
    bool found=false;
    std::vector<float> temp10=(*results1)[i];

    for(int j=0;j<(*results2).size();j++){
      if((*results1)[i][7]==(*results2)[j][7]){
        found=true;
      }
    }
    if(!found){
      temp10[8]=-1;
      temp10[6]++; // this is a little bit different,
      //so the function above is when results2 have something unmatch,
      //but we still want to print it out, but if the case is
      //if results1 have objects already found a match but it is not exist in resutls2

      (*results2).push_back(temp10);
